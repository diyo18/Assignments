...
For this one I kept track of the cheapest price seen so far. So when the next price comes, I compare it to see whether is a new cheapest price or a selling point. If it is a selling point, I subtract the cheapest day value and store it as the profit. Once it goes through all the numbers, I can then see the best profit.
```
#include <vector>
...
This problem was different because it had a limited amount of acceptable values. That actually made it a lot easier. I ended up using a counting sort since there are only 21 possible values of temperature so that allows us to pass through the array once. It ends up looping through the input numbers only once so time complexity is O(N).
...
To get the O(N) time for this problem, I used a hash set like in problem 1 to avoid checking every single number. Once I had the array of numbers in the hashset, I began by going through the array and subtracting from the current value. I did this to see whether the value was inside of a sequence or was a unique number / start of a sequence (has N - 1 then it means its in the middle of a sequence / if it doesn't then it is the start). If the number is the start of a potential sequence, then the while loop checks for the next number. Every single time it finds a new value, the counter goes up. In the end, the longest counter gets saved and returned. In total, I only inserted the array once (hash set) and the rest was looking up and checking values.
